...
if (defined( 'WPINC' ) ) {
	...
	function vjtt_settings() {
		...
		register_setting( 'vjtt-settingsgroup', 'vjtt_allowedsites' );
		register_setting( 'vjtt-settingsgroup', 'vjtt_watermarkoffsetw' );
		register_setting( 'vjtt-settingsgroup', 'vjtt_watermarkoffseth' );
	}

	function vjtt_settingspage(){
	?>
	<div class="wrap">
	<h1>Timthumb Settings</h1>
	<form method="post" action="options.php">
		<?php settings_fields( 'vjtt-settingsgroup' ); ?>
		<?php do_settings_sections( 'vjtt-settingsgroup' ); ?>
		<table class="form-table">
			<tr valign="top"><th scope="row">Watermark File</th><td><input type="text" name="vjtt_watermarkfile" value="<?php echo esc_attr( get_option('vjtt_watermarkfile') ); ?>" /></td></tr>
			<tr valign="top"><th scope="row">Watermark Offset W</th><td><input type="text" name="vjtt_watermarkoffsetw" value="<?php echo esc_attr( get_option('vjtt_watermarkoffsetw') ); ?>" /></td></tr>
			<tr valign="top"><th scope="row">Watermark Offset H</th><td><input type="text" name="vjtt_watermarkoffseth" value="<?php echo esc_attr( get_option('vjtt_watermarkoffseth') ); ?>" /></td></tr>
			<tr valign="top"><th scope="row">Allowed Sites</th><td><input type="text" name="vjtt_allowedsites" value="<?php echo esc_attr( get_option('vjtt_allowedsites') ); ?>" /></td></tr>
		</table>
		<?php submit_button(); ?>
	</form>
	</div>
	...
	function get_option($option){
		global $db;
		global $table_prefix;
		$result = $db->query("SELECT option_value FROM {$table_prefix}options WHERE option_name = '{$option}' LIMIT 1");
	...
	if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', 10485760*12);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
	...
	//if(! defined('WATERMARK_OVERLAY_IMAGE') )     define( 'WATERMARK_OVERLAY_IMAGE', 'ogwm2017crop.png');
	//if(! defined('WATERMARK_OVERLAY_IMAGE') )	define( 'WATERMARK_OVERLAY_IMAGE', 'watermark.png');
	...
	class timthumb {
		...
		protected function serveInternalImage(){
			...
			if($fileSize > MAX_FILE_SIZE){
				$this->error("The file you specified is greater than the maximum allowed file size {MAX_FILE_SIZE}.");
				return false;
			}
			if($fileSize <= 0){
		...
		protected function processImageAndWriteToCache($localImage){
			...
			$settext = $this->param('settext', null);
			...
			if ($watermark == 1) {
				...
				imagecopy( $canvas, $overlay_gd_image, $new_width - $overlay_width - WATERMARK_OFFSET_W, $new_height - $overlay_height - WATERMARK_OFFSET_H, 0, 0, $overlay_width, $overlay_height);
				imagealphablending($canvas, false );
				imagesavealpha($canvas , true);
			}
		...
		protected function sendImageHeaders($mimeType, $dataSize){
			...
			if(BROWSER_CACHE_DISABLE){
				...
				header('Cache-Control: public, s-maxage=' . PROXY_CACHE_MAX_AGE . ', max-age=' . BROWSER_CACHE_MAX_AGE);
